...
const EditUser = (props) => {
    ...
    useEffect(() => {
        ...
        const fetchUserData = async () => {
            try {
                const response = await fetch(`http://192.168.0.205:5001/api/contacts/${id}`);
    ...
    const onSubmit = async (data) => {  // Pass data explicitly
        ...
        try {
            const response = await fetch(`http://192.168.0.205:5001/api/contacts/update_contact/${data.id}`, request_options);
